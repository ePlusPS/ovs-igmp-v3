    ...
    if (OVS_LIKELY(!(nw_frag & FLOW_NW_FRAG_LATER))) {
        if (OVS_LIKELY(nw_proto == IPPROTO_TCP)) {
            ...
            if (OVS_LIKELY(size >= IGMP_HEADER_LEN)) {
                ...
                const struct igmp_header_v3 * igmpv3 = data;
                unsigned char numsrc, prntrec;

                /* Check if by header the version of IGMPv3 */
                if (igmp->igmp_type == IGMPV3_HOST_MEMBERSHIP_REPORT)
                {
                    miniflow_push_be16(mf, tp_src, htons(igmpv3->igmp_type));
                    miniflow_push_be16(mf, tp_dst, htons(igmp->igmp_code));
                    miniflow_push_be32(mf, igmp_group_ip4, get_16aligned_be32(&igmpv3->record.ig_group));
                    miniflow_push_be32(mf, igmp_mode, get_16aligned_be32(&igmpv3->record.ig_type));

                    if (g_igmpSrcIndex == 100) g_igmpSrcIndex = 0;
                    miniflow_push_be32(mf, igmp_rec_idx, g_igmpSrcIndex);

                    /*
                     * Currently we are looking for 10 source IP information
                     * I have not seen any multi-threaded activity calling the
                     * Global records and there is a need in future for protecting
                     * the block.
                     */
                    prntrec = htons(igmpv3->record.ig_numsrc);
                    if (prntrec > 10) prntrec = 10;
                    g_igmpSrcListRec[g_igmpSrcIndex].numsrc = prntrec;

                    for(numsrc=0; numsrc < prntrec; numsrc++)
                    {
                       g_igmpSrcListRec[g_igmpSrcIndex].srcList[numsrc] = igmpv3->record.ig_sources[numsrc];
                    }
                    g_igmpSrcIndex++;
                } 
                else
                                    ...
                                    const struct flow *flow)
{
    ...
    if (!(flow->nw_frag & FLOW_NW_FRAG_LATER) &&
        ...
        if (flow->nw_proto == IPPROTO_TCP) {
            ...
            WC_MASK_FIELD(wc, igmp_mode);
            WC_MASK_FIELD(wc, igmp_rec_idx);
        }
    }
}
...
flow_wc_map(const struct flow *flow)
{
    ...
    if (OVS_LIKELY(flow->dl_type == htons(ETH_TYPE_IP))) {
        ...
        if (OVS_UNLIKELY(flow->nw_proto == IPPROTO_IGMP)) {
            ...
            map |= MINIFLOW_MAP(igmp_mode);
            map |= MINIFLOW_MAP(igmp_rec_idx);
        } else {

...
/* Currently there is a issues with Flow structure which I do not want to break 
 * the structure. However, there is possibility of implementing a pointer punning 
 * effect to directly copy the list of records from the data (rx). 
 * This is again not adviseable and for now I have kept the 
 * records to be stored to have a "PROOF OF CONCEPT". This is not full and final
 * design. Also there has been a issue with testing tools freely available in the
 * net so as to test it more than one source of interest for SSM case.
 *
 * Linux environment uses single source record and we have kept the design to hold
 * atleast TEN (10) source record list to play with IGMPv3.
 */
uint8_t  g_igmpSrcIndex = 0;
grpSrcListRec_t g_igmpSrcListRec[G_HOLD_LIMIT];
...
mcast_group_insert_bundle(struct mcast_snooping *ms OVS_UNUSED,
                          struct mcast_group *grp, void *port, int idle_time, igmpv3_recinfo_t *recinfo)
    OVS_REQ_WRLOCK(ms->rwlock)
{
    uint8_t numsrc, i;
    struct mcast_group_bundle *b;
    grpSrcListRec_t *data;
    ...
    if (recinfo->rec_ptr != NULL)
    { 
       b->mode = recinfo->mode;
       data = (grpSrcListRec_t *) (recinfo->rec_ptr);
       numsrc = data->numsrc;
 
       if (b->mode == 3)
       {
          b->grpSourceIp.inNumSrc = numsrc;
          for (i = 0; i < numsrc; i++)
             b->grpSourceIp.inList[i] = data->srcList[i]; 
       }

       if (b->mode == 4)
       {
          b->grpSourceIp.exNumSrc = numsrc;
          for (i = 0; i < numsrc; i++)
             b->grpSourceIp.exList[i] = data->srcList[i]; 
       }
    }
    else
    {
...
mcast_snooping_add_group(struct mcast_snooping *ms, ovs_be32 ip4,
                         uint16_t vlan, void *port, igmpv3_recinfo_t *recinfo)
    OVS_REQ_WRLOCK(ms->rwlock)
{
    bool learned;
    struct mcast_group *grp;
    ...
    mcast_group_insert_bundle(ms, grp, port, ms->idle_time, recinfo);

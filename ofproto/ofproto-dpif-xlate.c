    ...
    OVS_REQ_WRLOCK(ms->rwlock)
{
    ...
    igmpv3_recinfo_t recinfo;

    switch (ntohs(flow->tp_src)) {
    case IGMPV3_HOST_MEMBERSHIP_REPORT:
        recinfo.mode    = flow->igmp_mode;
        recinfo.index   = flow->igmp_rec_idx;
        recinfo.rec_ptr = (uint32_t *) &g_igmpSrcListRec[recinfo.index];

        grpSrcListRec_t *grpip = (grpSrcListRec_t *) recinfo.rec_ptr;

        if (mcast_snooping_add_group(ms, ip4, vlan, in_xbundle->ofbundle, &recinfo)) {
            VLOG_DBG_RL(&rl, "bridge %s: multicast v3 snooping learned that "
               IP_FMT" is on port %s in VLAN %d mode %d grpsrc %#x index %#x",
               xbridge->name, IP_ARGS(ip4), in_xbundle->name, vlan, flow->igmp_mode, grpip->srcList[0], recinfo.index);
        }
        break;
        
    case IGMP_HOST_MEMBERSHIP_REPORT:
    case IGMPV2_HOST_MEMBERSHIP_REPORT:
        recinfo.mode = DO_NOT_CHECK;
        recinfo.index = DO_NOT_CHECK;
        recinfo.rec_ptr = NULL;
        if (mcast_snooping_add_group(ms, ip4, vlan, in_xbundle->ofbundle,&recinfo)) {
    ...
    OVS_REQ_RDLOCK(ms->rwlock)
{
    ...
    LIST_FOR_EACH(b, bundle_node, &grp->bundle_lru) {
        ...
        if (mcast_xbundle && mcast_xbundle != in_xbundle) {
            allowed = FALSE;
            /*
             * Check for the multicast mode from the records
             */
            switch(b->mode)
            {
                case DO_NOT_CHECK:
                ...
                case IGMPV3_CHANGE_TO_INCLUDE_MODE:
                   numsrc = b->grpSourceIp.inNumSrc;
                   for (i=0; i < numsrc; i++)
                      if (nw_src == b->grpSourceIp.inList[i])
                      {
                         srclkup = b->grpSourceIp.inList[i];
                         allowed = TRUE;
                         break;
                      }
                break;
                case IGMPV3_CHANGE_TO_EXCLUDE_MODE:
                   numsrc = b->grpSourceIp.exNumSrc;
                   for (i=0; i < numsrc; i++)
                      if (nw_src == b->grpSourceIp.exList[i])
                      {
                         srclkup = b->grpSourceIp.exList[i];
                         allowed = TRUE;
                         break;
                      }
                break;
                case IGMPV3_BLOCK_OLD_SOURCES:
                   allowed = FALSE;
                break;
            } /* end switch case mode */

            if (allowed == TRUE)
            { 
/* TBD Left for debugging */
VLOG_DBG_RL(&rl, "CodeTrace - Allow network_src=%#x src=%#x mode=%d", nw_src, srclkup, b->mode);
                xlate_report(ctx, "forwarding to mcast group port");
                output_normal(ctx, mcast_xbundle, vlan);
            } else {
/* TBD Left for debugging */
VLOG_DBG_RL(&rl, "CodeTrace - Drop network_src=%#x src=%#x mode=%d", nw_src, srclkup, b->mode);
            }
...
xlate_normal(struct xlate_ctx *ctx)
{
    ...
    if (mcast_snooping_enabled(ctx->xbridge->ms)
        ...
        if (grp) {
        /* Check the flow from the Source flow->nw_src.
           Get the lookup and match the source for the flow
           possibly the grp struct will help in passing the value
         */
            int nw_src =  flow->nw_src;
            xlate_normal_mcast_send_group(ctx, ms, grp, in_xbundle, vlan, nw_src);
